...
namespace ModengTerm.ViewModel.Terminal
{
    ...
    public class PlaybackSessionVM : ViewModelBase
    {
        ...
        public int Open(PlaybackOptions options)
        {
            ...
            playbackEvent = new AutoResetEvent(false);
            ...
            videoTerminal = new VideoTerminal();
            videoTerminal.Initialize(vtOptions);

            #endregion

            playbackTask = Task.Factory.StartNew(PlaybackThreadProc);
        ...
        private void SessionTransport_DataReceived(SessionTransport client, byte[] bytes, int size)
        {
            System.Windows.Application.Current.Dispatcher.Invoke(() =>
            {
                try
                {
                    videoTerminal.ProcessRead(bytes, size);
                }
                catch (Exception ex)
                {
        ...
        private void PlaybackThreadProc()
        {
            ...
            int code = playbackStream.OpenRead(playback);
