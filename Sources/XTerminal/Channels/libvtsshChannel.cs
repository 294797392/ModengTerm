...
namespace VideoTerminal.Channels
{
    public class libvtsshChannel : VTChannel
    {
        #region 类变量
        ...
        public libvtsshChannel(VTInitialOptions options) :
            base(options)
        {
            ...
            SSHChannelAuthorition channelAuthorition = this.options.Authorition as SSHChannelAuthorition;
            ...
            libvt.vtssh_options vtssh_Options = new libvt.vtssh_options()
            {
                ...
                term = Encoding.ASCII.GetBytes(XTermUtils.GetTermName(terminalOptions.Type)),
                term_columns = terminalOptions.Columns,
                term_rows = terminalOptions.Rows,
        ...
        private VTChannelState GetVTChannelState(libvt.vtssh_status_enum status)
        {
            switch (status)
            {
                case libvt.vtssh_status_enum.VTSSH_STATUS_CONNECTED: return VTChannelState.Connected;
                case libvt.vtssh_status_enum.VTSSH_STATUS_CONNECTING: return VTChannelState.Connecting;
                case libvt.vtssh_status_enum.VTSSH_STATUS_DISCONNECTED: return VTChannelState.Disconnected;
                default:
